...
typedef uint64_t   branch_t;
typedef uint64_t   rvjit_addr_t;
...
typedef struct {
    ...
    // Block exit paths
    vector_t(struct {rvjit_addr_t dest; uint64_t ptr;}) links;
...
bool rvjit_ctx_init(rvjit_block_t* block, uint64_t heap_size);
...
static inline void rvjit_put_code(rvjit_block_t* block, const void* inst, uint64_t size)
{
